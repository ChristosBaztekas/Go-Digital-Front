...
export const RegisterPopup = ({ handleLoginPopupClose, onSwitchToOtp }) => {
    ...
    return (
        <div className="py-8 px-6 sm:py-9 sm:px-12 md:px-24 max-w-[890px] w-full relative bg-[#F1EEEC] rounded-[39px]">
            ...
            <h2 className="text-center text-black text-3xl sm:text-4xl font-semibold max-w-[430px] mx-auto pt-4 sm:pt-0">{t('auth.registerScreen.titleMain')}</h2>
            <h3 className="text-xl sm:text-2xl font-bold mt-8 px-2">{t('auth.registerScreen.titleForm')}</h3>
            <p className="text-base sm:text-lg font-medium mt-1 px-2 text-gray-700">{t('auth.registerScreen.textIntro')}</p>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-4">
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 w-full">
                    <input
                        ...
                        placeholder={t('auth.registerScreen.inputName')}
                    ...
                    <input
                        ...
                        onChange={handleInputChange}
                        placeholder={t('auth.registerScreen.inputSurname')}
                        className="rounded-[10px] border border-stone-300 w-full px-4 sm:px-6 py-3 sm:py-4 text-base focus:outline-none focus:ring-2 focus:ring-[#F15D2A] focus:border-transparent transition"
                        required
                    />
                </div>
                ...
                <input
                    ...
                    onChange={handleInputChange}
                    placeholder={t('auth.registerScreen.inputEmail')}
                    className="rounded-[10px] border border-stone-300 w-full px-4 sm:px-6 py-3 sm:py-4 text-base focus:outline-none focus:ring-2 focus:ring-[#F15D2A] focus:border-transparent transition"
                    required
                />
                ...
                <div className="w-full">
                    <PhoneInput
                        ...
                        placeholder={t('auth.registerScreen.inputPhone')}
                        enableSearch={true}
                        countryCodeEditable={false}
                        searchPlaceholder={t('auth.registerScreen.phoneSearch')}
                ...
                <input
                    ...
                    onChange={handleInputChange}
                    placeholder={t('auth.registerScreen.inputAddress')}
                    className="rounded-[10px] border border-stone-300 w-full px-4 sm:px-6 py-3 sm:py-4 text-base focus:outline-none focus:ring-2 focus:ring-[#F15D2A] focus:border-transparent transition"
                    required
                />
                ...
                >
                    {t('auth.registerScreen.buttonSubmit')}
                </button>
            </form>
        </div>
    );
